...
for(const file of ['synthetic-data.js','deal-analysis.js','document-upload-ui.js']){
  const tracedPath=join(process.cwd(),'lib','synthetic',file);
  const source=existsSync(tracedPath)?readFileSync(tracedPath,'utf8'):readFileSync(new URL('./'+file,import.meta.url),'utf8');
