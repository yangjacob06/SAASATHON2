> **Live summary integration prepared (27 September 2026).** The hosted pitch demo now has code for a private `/api/analyze` server function using OpenAI Responses. After CSV review, the adviser can generate a live draft, review/edit it and complete the final confirmation. An explicitly selected local template remains available. CSV parsing, graphs, lender criteria fit and PDFs retain their existing calculation rules. Deployment and server secrets are still required; this note does not claim the hosted API is configured or verified. See [live setup](../../docs/LIVE_DEMO_API.md). Earlier descriptions of the local-only implementation below are historical.

> **Workspace navigation update (27 September 2026).** Overview is a dashboard with live session totals, deal pipeline, next steps and activity. Deals is a searchable register. Lenders is an independent, searchable directory of the five fictional lender relationships and their mandates. The three original companies are preloaded examples of reviewed/submitted deal packs, explicitly simulated; their original FY2025 figures and missing documents are preserved. No earlier years, actual communications or lender approvals are invented. Southern Manufacturing Demo Ltd remains the $8m CSV pitch journey. Notes and stage changes remain local, and reset restores the three preloaded examples. See [review notes](../../docs/PROTOTYPE_REVIEW.md).
...
> **Local pitch update (27 September 2026).** The HTML prototype now guides the adviser through CSV comparison, a final confirmation summary, and five interactive lender cards with individual report downloads. The requested fit percentage is explicitly **criteria alignment** (aligned checks divided by all seven checks, including unknowns), not approval probability or a credit score. Annual revenue, EBITDA, cash-flow and profit charts use uploaded synthetic data. See [PITCH.md](PITCH.md) for the walkthrough and data sources. Analysis remains local and deterministic; no OpenAI/API integration has been added. The implementation descriptions below record the earlier milestones.
